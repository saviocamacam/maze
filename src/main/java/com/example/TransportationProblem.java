...
public class TransportationProblem {
    ...
    static LinkedList<Shipment> matrixToList() {
        return stream(matrix).flatMap(row -> stream(row)).filter(s -> s != null).collect(toCollection(LinkedList::new));
    }
    ...
    static void updateFile(Map content) throws IOException {
        ...
        BufferedWriter writer = new BufferedWriter(new FileWriter("src/main/java/com/example/input0.txt"));
        try {
            JSONObject json = new JSONObject(content);

            System.out.println("Erro");
            for (Object var : json.getJSONArray("teste")) {
